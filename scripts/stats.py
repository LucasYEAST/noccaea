...
def get_layer(plant_multimsk, colors, substructure):
    "Get binary mask for substructure from multimsk"
...
def get_sub_ele_img(plant_ele_img, sub_msk):
    "Get metal image with all pixels at 0 except those under plant mask" 
    sub_msk_bool = sub_msk > 0
    return(sub_msk_bool * plant_ele_img)

def get_sub_ele_stats(sub_ele_img):
    "Get statistics from metal image"
...
def XrandPixel_value(layer_msk, img, fn, subs, X):
    "Sample X random pixels under a binary mask and return their sum"
    # Get indices of mask
